...
The following example demonstrates the automatic attachment of the necessary HTML markup. In this case you can omit the manual insertion of HTML markup completely. Everything is handled by the plugin.
...
In the above example the overlay will only be shown when the user not already saw it within the cookie expiry period. Please mind that you use a selector that fits your implementation. If you want to always show the overlay in dependence of a click use the following configuration:
...
You have to configure the plugin as follows if you don't want that the HTML is generated by the plugin:
